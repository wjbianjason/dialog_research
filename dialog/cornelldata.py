    ...
    def getTrainRawData(self):
        ...
        for conversation in conversations:
            for i in range(len(conversation["lines"]) - 1):  # We ignore the last line (no answer for it)
                ...
                TrainRawData.append((inputLine["text"],targetLine["text"]))
        return TrainRawData
if __name__ == "__main__":
    cc = CornellData("../data/cornell/")
    data = cc.getTrainRawData()
    # print(data)
    of = open("../data/cornell/movie_conv.txt",'w')
    for a in data:
        of.write(a[0].strip()+"\t"+a[1].strip()+"\n")
    of.close()
