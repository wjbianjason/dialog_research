...
class BSDecoder(object):
  ...
  def __init__(self, model, batch_reader, hps, vocab,config_param):
    ...
    self._decode_io = DecodeIO(self._config_param.decode_dir)
    self._data_generator = batch_reader._data_generator

  def DecodeLoop(self):
    """Decoding loop for long running process."""
    sess = tf.Session(config=tf.ConfigProto(allow_soft_placement=True))
    step = 0
    while step < self._config_param.max_decode_steps:
  ...
  def _Decode(self, saver, sess):
    ...
    """
    ckpt_state = tf.train.get_checkpoint_state(self._config_param.log_root)
    if not (ckpt_state and ckpt_state.model_checkpoint_path):
      tf.logging.info('No model to decode yet at %s', self._config_param.log_root)
      return False

    tf.logging.info('checkpoint path %s', ckpt_state.model_checkpoint_path)
    ckpt_path = os.path.join(
        self._config_param.log_root, os.path.basename(ckpt_state.model_checkpoint_path))
    tf.logging.info('renamed checkpoint path %s', ckpt_path)
    saver.restore(sess, ckpt_path)

    self._decode_io.ResetFiles()
    for _ in xrange(self._config_param.decode_batches_per_ckpt):
      ...
      for i in xrange(self._hps.batch_size):
        bs = beam_search.BeamSearch(
            ...
            self._vocab.WordToId(self._data_generator.SENTENCE_START),
            self._vocab.WordToId(self._data_generator.SENTENCE_END),
  ...
  def _DecodeBatch(self, article, abstract, output_ids):
    ...
    """
    decoded_output = ' '.join(self._data_generator.Ids2Words(output_ids))
    end_p = decoded_output.find(self._data_generator.SENTENCE_END, 0)
    if end_p != -1:
