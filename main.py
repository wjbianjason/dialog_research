...
def tensorflow_main(mode):
	...
	from dialog.data_process import Vocab,DataGenerator
	from dialog.batcher import Batcher
	import tensorflow as tf
	tf.logging.set_verbosity(tf.logging.INFO)
	#data ready
	_vocab = Vocab(vocab_file="./data/vocab/vocab_output.txt",max_size=18000)
	model_param = ModelParam(mode=mode,batch_size=64,enc_layers=2,num_softmax_samples=4096,\
	enc_timesteps=50,dec_timesteps=18,min_input_len=20,min_output_len=7) #most has been set default
	data_generator = DataGenerator(vocab=_vocab,modelParam=model_param,word2vec=None)
	data_generator.loadCorpus("./data/train/summary_corpus_train.txt",iterFlag=True)
	batcher = Batcher(dataGenerator=data_generator, bucketing=True, truncate_input=False)

	config_param = ConfigParam(num_gpus=1,epoch=8) #all has been set default
...
def keras_main(mode):
	...
	model_param = ModelParam(enc_timesteps=50,dec_timesteps=18,\
		min_input_len=20,min_output_len=7,batch_size=64)
